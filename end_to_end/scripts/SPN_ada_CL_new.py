...
from stage_SPN import StageWorld
import random
import rospy
import os
import signal
...
#    min_q_pi = tf.minimum(min_q_pi, 24.0)
    ...
    regularizer = tf.keras.regularizers.l2(0.001)     # 定义正则化器 
    all_trainable_weights_pi = [var for var in tf.trainable_variables() if 'main/pi' in var.name]     # 获取可训练变量 
    regularization_penalty_pi = tf.add_n([regularizer(var) for var in all_trainable_weights_pi])     # 应用正则化 
...
#    test_result_plot = np.load('test_result_plot90888.npy')
    suc_record_all = np.zeros((5,130,8))
    suc_record_all_new = np.zeros((5,130,8))
    test_result_plot = np.zeros((5,50,130,5))
...
#    np.save('test_result_plot'+str(sac)+'.npy',test_result_plot)
    ...
    for hyper_exp in range(5):
        ...
        past_env = [0]
        current_env=[0]
        suc_record = np.zeros((8,50))
        suc_record1 = np.zeros((8,50))
        suc_record2 = np.zeros((8,50))
        suc_pointer = np.zeros(8)
        mean_rate = np.ones(8)
        p = np.zeros(8)
        p[0]=1.0
        mean_rate[0]=0.0
        ...
        trainable_saver = tf.train.Saver(trainables,max_to_keep=None)
        sess.run(tf.global_variables_initializer())
#        trainable_saver.restore(sess,"/home/zhw1993/Monocular-Obstacle-Avoidance/D3QN/paper3_train/networks/sac139lambda0-499")
        episode=0
        T = 0
        epi_thr = 0
        goal_reach=0
        ...
        b_test = True
        while test_time<101:       
            if succ_rate_test>=0.9 and episode > start_epoch and new_env<7:
                new_env = new_env+1 
                succ_rate_test = 0
                mean_rate[new_env] = 0.0
...
#                    robot_size = env.ResetWorld(env_no)
                    ...
                    #env.Control([-1,0.1])
                    rate.sleep()
                print("train env no. is",env_no,"selecting p",p,'target distance',r2gd)
                reset = False
                    ...
                    if d:
                        if episode > start_epoch:
                            ...
                            if env.stop_counter < 1.0:
                                suc_record1[env_no,int(suc_pointer[env_no])] = goal_reach
                                suc_record2[env_no,int(suc_pointer[env_no])] = 0
                            else:
                                suc_record1[env_no,int(suc_pointer[env_no])] = 0.0
                                suc_record2[env_no,int(suc_pointer[env_no])] = 0
                        ...
                        if ep_len >= max_ep_len: 
                            ...
                            if episode > start_epoch:
                                suc_record[env_no,int(suc_pointer[env_no])] = goal_reach
                                suc_record1[env_no,int(suc_pointer[env_no])] = goal_reach
                                suc_record2[env_no,int(suc_pointer[env_no])] = 1.0
                                mean_rate[env_no] = np.mean(suc_record[env_no,:])
                                suc_pointer[env_no]=(suc_pointer[env_no]+1)%50
    ...
    #                            print(batch['obs1'])
                                ...
                                if T%10000==0:
                                    for k in range(50):
                                        ...
                                        ep_len_test = 0
                                        rospy.sleep(2.0)
                                        env.set_robot_pose_test(k+new_env*50)
                                        env.GenerateTargetPoint_test(k+new_env*50)
                                        o, r, d,goal_reach,r2gd,robot_pose =env.step()
                                        for i in range(500):
                                        ...
                                        #                                    replay_buffer.store(o, a, r, o2, d)
                                            ...
                                            if d or (ep_len_test >= max_ep_len):
                                                test_result_plot[hyper_exp,k,test_time,0] = return_epoch_test
                                                test_result_plot[hyper_exp,k,test_time,1] = goal_reach
                                                test_result_plot[hyper_exp,k,test_time,2] = ep_len_test  
                                                test_result_plot[hyper_exp,k,test_time,3] = 1.0*goal_reach - 0.0025*ep_len_test
                                                test_result_plot[hyper_exp,k,test_time,4] = env.crash_stop
                                                if k==49:
                                                    succ_rate_test = np.mean(test_result_plot[hyper_exp,:,test_time,1])
                                                    print("success rate in"+str(50)+"is:",succ_rate_test)
                    ...
                    #                                                trainable_saver.save(sess, '/home/zhw1993/Monocular-Obstacle-Avoidance/D3QN/input_compare/ETH/networks/sac'+str(sac)+'lambda'+str(hyper_exp), global_step=test_time)
                                                    ...
                                                    suc_record_all[hyper_exp,test_time,:] = np.mean(suc_record1,axis=1)
                                                    suc_record_all_new[hyper_exp,test_time,:] = np.mean(suc_record2,axis=1)
                                                    np.save('suc_record_all'+str(sac)+'.npy',suc_record_all) 
                                                    np.save('suc_record_all_new'+str(sac)+'.npy',suc_record_all_new) 
                                                    np.save('test_result_plot'+str(sac)+'.npy',test_result_plot)
                                                    if (test_time) % 5== 0:
                                                        trainable_saver.save(sess, '/home/eias/20240118/Monocular-Obstacle-Avoidance/D3QN/paper3_train/networks/sac'+str(sac)+'lambda'+str(hyper_exp), global_step=test_time)
                                                        rospy.sleep(3.0)
                                                    b_test = False
                                                break                                
...
#                summary_writer.add_summary(summary_str, episode - start_epoch)        
            ...
            # save progress every 500 episodes
                ...
                if episode%5==0:
                    print("EPISODE"+str(sac)+str(hyper_exp), episode, "/ REWARD", return_epoch, "/ steps ", T,"success rate",np.mean(suc_record,axis=1),"timeout rate",np.mean(suc_record_all,axis=2)[hyper_exp,max(0,test_time-10):test_time+1],"total suc rate2",np.mean(suc_record_all_new,axis=2)[hyper_exp,max(0,test_time-10):test_time+1])
                episode = episode + 1
                epi_thr = epi_thr+1
...
if __name__ == '__main__':
	...
	subprocess.Popen(["rosrun","stage_ros1", "stageros", "/home/eias/20240118/Monocular-Obstacle-Avoidance/D3QN/paper3_dot/worlds/d8888.world"])
