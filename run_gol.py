...
def run_waypoint_script(): 
    subprocess.Popen(["python", "/home/eias/end2end/jackal_ws/src/barn-EIT-NUS/end_to_end/scripts/waypoint.py"]) 
    print("waypoint.py started.")
...
if __name__ == "__main__":
    ...
    e2e_process = subprocess.Popen([
        'python3',
        "end_to_end/scripts/test_policy_sac.py",
        '--world_id=' + str(args.world_idx)
    ])
    time.sleep(3.5)
        # 0. Start waypoint.py in a separate thread
    waypoint_thread = threading.Thread(target=run_waypoint_script)
    waypoint_thread.start()
#    2. move_base_DWA.launch
    launch_file = join(base_path, '..', 'jackal_helper/launch/move_base_gamp.launch')
    ...
    mb_goal.target_pose.pose.position.x = GOAL_POSITION[1]
    mb_goal.target_pose.pose.position.y = GOAL_POSITION[0]
    ...
    os.system('pkill -9 pt_main_thread')
